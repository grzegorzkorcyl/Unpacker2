...
void Unpacker2::DistributeEventsSingleStep() {
  ...
  if (file->is_open()) {
    ...
    while (true) {
      ...
      if (debugMode == true)
        cerr
            << " current event size " << eventSize << endl
            << "Unpacker2.cc: Starting new event analysis, going over subevents"
            << endl;
      ...
      while (true) {
        ...
        if ((*data) != 0) {
          ...
          if (debugMode == true) {
            cerr << "Unpacker2.cc: Processing event " << analyzedEvents
                 << " on " << getHubAddress() << endl;
            cerr << "Unpacker_TRB3.cc: Receiving " << dataSize
                 << " words to analyze" << endl;
          }
          ...
          while (dataSize > 0) {
            ...
            if (tdc_offsets.count(tdcNumber) > 0) {
              ...
              while (is > 0) {
                ...
                case 4: // time data
                  if (channel >= 0 &&
                      (unsigned(channel) != ((data_i >> 22) & 0x7f))) {
                    firstHitOnCh = true;
                  }
                    ...
                    if (fine == prev_fine && coarse == prev_coarse &&
                      ...
                      if (repetition_counter > kMaxAllowedRepetitions) {
                        ...
                        cerr << "WARNING: corrupted data detected on channel "
                             << tmp_ch
                             << ". Skipping event data for this channel."
                             << endl;
                      }
                  ...
                  if (useTDCcorrection == true &&
                    ...
                    fine =
                        TDCcorrections[channel + channelOffset]->GetBinContent(
                            fine + 1);
                  } else {
                    fine = fine * 10;
                  }
                    ...
                    if (channel == 0) {
                      ...
                      if (gotRef == true) {
                        ...
                        if (firstHitOnCh == true) {
                          new_ch =
                              eventIII->AddTDCChannel(channel + channelOffset);
                        }

                        fullTime = fullTime - refTime;

                        if (isRising == false) {
                          fullTime -= TOTcalibHist->GetBinContent(
              ...
              if (debugMode == true)
                cerr << "Unpacker_TRB3.cc: No Unpacker found for module "
                     << tdcNumber << " skipping " << internalSize << " bytes"
                     << endl;
          ...
          cerr << "WARNING: First data word empty, skipping event nr "
               << analyzedEvents << endl;
        }

        if (debugMode == true)
          cerr << "Unpacker2.cc: Ignoring " << (getPaddedSize() - getDataSize())
               << " bytes and reducing eventSize by " << getDataSize();
        ...
        if (eventSize > initialEventSize) {
          cerr << "ERROR: Incorrect event size, the input file is likely "
                  "corrupted."
               << endl;
          cerr << "Stopping event loop after " << analyzedEvents
               << " events read." << endl;
          break;
        }
      ...
      if (debugMode == true) {
        cerr << "Unpacker2.cc: Ignoring padding of the event "
             << (align8(eventSize) - eventSize) << endl;
        cerr << "Unpacker2.cc: File pointer at " << file->tellg() << " of "
             << fileSize << " bytes" << endl;
      }
    ...
    TObjString timestamp(TimeDateDecoder::formatTimeString(
                             ((EventHdr *)pHdr)->date, ((EventHdr *)pHdr)->time)
                             .c_str());
    timestamp.Write();
...
bool Unpacker2::loadTDCcalibFile(const char *calibFile) {
  ...
  if (f->IsOpen()) {
    ...
    if (debugMode) {
      cerr << "The TDC calibration file " << calibFile
           << " could not be properly opened." << endl;
